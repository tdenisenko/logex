...
conversion. They select each partition's ordered prefix before reading payloads;
the exact filter and overlapping-range rules remain in effect. Stable ordering
uses an owned index permutation and in-place row moves, avoiding unaccounted
sorting scratch. Already ordered rows need no permutation. Cancellation is checked
between column reads and during row construction/permutation; an admitted column
read or index sort remains a synchronous operation. The explicit budgeted native
...
Native sums over payload data without grouping, conditional inputs or residual
expressions account their candidate IDs, retained source buffers, batches and
numeric working memory. Exact native filters still apply. Raw buffers and page
indexes are loaded once per partition scan; companion pages are reused across
batch boundaries. Repeated sums of the same selected payload sequence share one
accumulator. Arithmetic, merging, projection and decimal-formatting allowances
retain input ownership while admitting replacement and temporary buffers, with
batch admission instead of a shared-budget lock for every row. Numeric allowances
are conservative and preserve the existing optimized arithmetic and formatter.
They depend on audited `num-bigint` 0.4.6 and pinned Rust allocation behavior;
changing either requires reviewing these bounds. Cancellation is checked between
...
before construction, including results from native SQL shortcuts. Their immutable
owner retains the charges through borrowing and protocol conversion. JSON object
nodes use a conservative allowance derived from the pinned Rust implementation;
...
| `query_memory_bytes` | positive integer | Shared SQL operator/candidate/source/scan-output/result accounted-memory budget in bytes during sync; default 1073741824. Explicit CLI values override config. Coverage limitations are described above. |
