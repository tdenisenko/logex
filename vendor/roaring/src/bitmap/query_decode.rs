...
mod tests {
    ...
    fn logex_runs_are_normalized_without_oversized_array_or_scratch() {
        for cardinality in [1, 4096, 4097, 5000] {
            ...
            // The generic run decoder can retain an oversized ArrayStore.
            // Upstream equality compares Store variants, so verify set semantics
            // independently of the normalized representation required here.
            let legacy = RoaringBitmap::deserialize_from(&bytes[..]).unwrap();
            assert_eq!(result.len(), legacy.len());
            assert!(result.iter().eq(legacy.iter()));
            assert!(
