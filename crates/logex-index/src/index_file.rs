...
impl IndexFile {
    ...
    /// Return the pending, unverified first-page bytes solely as a routing hint
    /// for a bounded whole-file verification. Callers must not derive success,
    /// absence, or decoded data from this prefix; every selected route must
    /// consume this same handle through `read_all` before exposing a result.
    pub(crate) fn pending_unverified_prefix_for_bounded_route(
        &self,
        max_logical_len: u64,
    ) -> Option<&[u8]> {
        (self.protected
            && self.prefetched_page
            && self.logical_len > PAGE_BYTES as u64
            && self.logical_len <= max_logical_len)
            .then_some(&self.page[..self.page_len])
    }
