...
async fn decimal_average_group_filter_and_sliding_controls() {
    ...
    let sql = format!(
        "SELECT i % 2 AS g, AVG(x) FILTER (WHERE i < 3) AS a, AVG(DISTINCT x) AS d FROM {table} GROUP BY i % 2 ORDER BY g"
    );
    let result = execute_sql(&sql, &storage, storage.head_block())
        .await
        .unwrap();
    ...
    let sql = format!(
        "SELECT AVG(x) OVER (ORDER BY i ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) AS a FROM {table} ORDER BY i"
    );
    let result = execute_sql(&sql, &storage, storage.head_block())
        .await
        .unwrap();
    ...
    let sql = format!(
        "SELECT AVG(DISTINCT x) OVER (ORDER BY i ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS a FROM {table} ORDER BY i"
    );
    let result = execute_sql(&sql, &storage, storage.head_block())
        .await
        .unwrap();
    ...
    let sql = format!(
        "SELECT AVG(DISTINCT x) OVER (ORDER BY i ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) AS a FROM {table}"
    );
    let error = execute_sql(&sql, &storage, storage.head_block())
        .await
        .unwrap_err();
...
async fn duration_average_large_subtotals() {
    ...
    for sql in [
        ...
        format!(
            "SELECT CAST(AVG(x) OVER (ORDER BY i ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) AS BIGINT) AS a FROM {table} ORDER BY i"
        ),
    ] {
        let error = execute_sql(&sql, &storage, storage.head_block())
            .await
            .unwrap_err();
        assert!(
            error.to_string().contains("AVG sum overflow"),
            "{sql}: {error}"
        );
    }
}
...
async fn duration_average_small_values_preserve_units() {
    ...
    for unit in ["Second", "Millisecond", "Microsecond", "Nanosecond"] {
        let table = format!(
            "(VALUES (0, arrow_cast(-4, 'Duration({unit})')), (1, arrow_cast(8, 'Duration({unit})')), (2, NULL)) t(i,x)"
        );
        for sql in [
            format!("SELECT CAST(AVG(x) AS BIGINT) AS a FROM {table}"),
            format!("SELECT CAST(AVG(x) AS BIGINT) AS a FROM {table} GROUP BY i % 1"),
