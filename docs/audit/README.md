...
| 2 | WAL, catalogs, segments, codecs, readers, durability | In progress: [WAL recovery integrity](wal-recovery.md) merged in PR #129 as `09a63f55`. [Journaled commit recovery](storage-commit-replay.md) remains in draft [PR #130](https://github.com/tdenisenko/logex/pull/130): checkpoint correctness/CI and [isolated ExFAT tests](baselines/2026-09-11-exfat.md) pass, but performance exceeds the user’s 10% limit. [Caller-level publication work](ingestion-publication.md) additionally reproduced an unresolved row/progress restart gap on both the original baseline and candidate. Remaining formats and persisted state are pending. |
