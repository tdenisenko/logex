...
| 4 | EL discovery, peer management, requests, serving cache | In progress: [execution peer persistence](execution-peer-persistence.md) bounds startup cache reads/records, preserves damaged hints and isolates staging writes. Implementation and validation in progress. Remaining live inventory, request ownership, attribution and cancellation review is open. |
