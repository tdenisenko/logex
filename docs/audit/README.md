...
| 6 | Indexes, bloom filters, index publication | In progress: [numeric range boundary fixes](sql-predicates.md) cover reversed ranges and inclusive maximum keys. [Derived-file integrity](index-file-integrity.md) now covers B-tree/bitmap structure, bloom page checks, captured source-row counts and legacy publication rebuilds. All nine final local gates pass at `b6076c00`; retained original-baseline comparisons and fixed confirmations record gains and remaining tail limits. PR CI/merge, whole-artifact source binding and the rest of this area remain pending. |
