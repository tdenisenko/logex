...
| 10 | CLI/runtime and volume supervision | [Node worker supervision](node-worker-supervision.md) fixes lost HTTP/gRPC and checkpoint errors and shutdown-unwind reporting; all eight final-source local gates pass (1,687 tests, 24 ignored); CI/merge pending. Directory exclusivity prerequisite implemented with journal recovery; remaining work pending: preflight volume identity and writability, runtime loss detection, launchd/systemd templates. |
