...
| 10 | CLI/runtime and volume supervision | [Cleanup deadlines and results](runtime-cleanup-deadlines.md) cover ordinary shutdown through runtime destruction and preserve cleanup failures; all eight local gates pass (1,700 tests, 24 ignored); CI/merge pending. [Node worker supervision](node-worker-supervision.md) fixes lost HTTP/gRPC and checkpoint errors and shutdown-unwind reporting; merged in PR #199 (`4915ad57`) after eight local gates (1,687 tests, 24 ignored) and six CI jobs. Directory exclusivity prerequisite implemented with journal recovery; remaining work pending: preflight volume identity and writability, runtime loss detection, launchd/systemd templates. |
