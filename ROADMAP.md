...
Latest live check reconfirmed the Mac mini is still using the VPS path correctly: public egress is `157.245.195.72`, default egress routes over WireGuard, LogEx advertises `--nat extip:157.245.195.72`, the VPS forwards EL/CL/dashboard ports, and public `/status` remains reachable. Low peer counts after restart are now explained by the cleaned known-peer cache rebuilding from roughly 80 dialable peers plus many low-quality discovery candidates, not by broken NAT or missing inbound reachability. A 2s body/receipt hedge-delay experiment improved p95 fetch latency but did not improve sustained logs/sec, so it was reverted and the accepted baseline was redeployed.
