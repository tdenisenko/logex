...
- Challenge: public EL IPv6 peers were effectively unavailable from the test droplet.
  ...
  - Remaining: current public IPv6 EL discovery is not deterministic; the latest post-fix official-ENR, TCP-open-ENR, Stakely-checkpoint, and refreshed-binary public proofs submitted thousands of strict-IPv6 EL dials without any accepted EL serving session, and geth 1.17.4 also formed zero peers in IPv6-only comparison runs on the same droplet. Strict IPv6 historical-sync performance parity remains unproven without reliable operator-provided IPv6 execution bootnodes or a stronger public IPv6 peer source.
