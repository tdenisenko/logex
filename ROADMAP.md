...
Latest Mac mini peer investigation reconfirmed the VPS path is healthy: public egress is `157.245.195.72`, default traffic routes over WireGuard, LogEx advertises the VPS `extip`, public `/status` works through the VPS, DNAT/FORWARD rules are present, and inbound EL sessions reach `10.66.0.2:30303`. Low peer windows are caused by slow conversion from connected/candidate peers into body/receipt-proven serving peers and request-tail timeouts. The accepted candidate now probes unproven body/receipt peers with smaller request limits and tightens adaptive request-size latency watermarks to `2s/3s`; the Mac mini benchmark improved recent logs/sec and body/receipt p95 versus the restored baseline without resetting the data directory.
...
- Validated the accepted timeout change with `cargo fmt --all -- --check`, `cargo test -p logex-sync p2p::peer_manager::requests`, and `cargo clippy -p logex-sync --all-targets -- -D warnings`.
- Tested and rejected a fetch-depth-only experiment. It raised average throughput but worsened p95 gaps and still produced zero-active-request windows while serving peers were available.
- Increased historical prepare lookahead and ready-batch drain from 2 to 4. The combined benchmark kept active requests primed more consistently and improved average/p50 logs/sec and p95 batch gaps versus the accepted baseline.
- Validated the scheduler smoothing change with `cargo fmt --all -- --check`, `cargo test -p logex-sync historical_fetch`, `cargo check -p logex-sync`, and `cargo clippy -p logex-sync --all-targets -- -D warnings`.
- Rechecked WireGuard/VPS while serving-peer counts were low. Public egress, default route, LogEx NAT flags, public dashboard access, VPS DNAT/FORWARD rules, and inbound EL sockets were all healthy, so the low-peer symptom was isolated to LogEx peer qualification/request scheduling.
- Confirmed the low-peer window is mostly candidate-to-serving conversion: connected peers warmed slowly while `connected + pending_dials` stayed near the configured target, with many queued candidates and few body/receipt-proven serving peers.
- Added smaller effective body/receipt request limits for unproven peers. Proven serving peers keep their adaptive limits, but compatible peers that have not yet served verified data are probed more cautiously.
- Tightened adaptive body/receipt request-size latency watermarks from `3s/5s` to `2s/3s`, matching the production-client pattern of treating multi-second body/receipt latency as a capacity signal.
- Validated the accepted peer-sizing candidate with `cargo fmt --all -- --check`, a focused `logex-sync` unit test, and `cargo check -p logex-sync`.
- Deployed the candidate to the Mac mini and benchmarked it without resetting `/Volumes/SSD 4TB/LogEx`. The accepted candidate improved the sampled body/receipt p95 from about `8.1s` to about `6.1s` and recent logs/sec from about `122k` to about `186k`; live status later showed about `236k` logs/sec with `23` connected and `15` serving peers.
- Retested the prior `2s` body/receipt hedge-delay idea on top of the accepted request-sizing baseline. It regressed the benchmark (`~101k` average logs/sec and `~15.4s` body/receipt p95 versus `~158k` and `~9.4s` on the accepted run), so it was reverted and the accepted binary was redeployed.
- Tested lowering the medium historical lookahead threshold from `16` to `12` serving peers on top of the accepted request-sizing baseline. It increased request pressure too early and regressed the benchmark (`~137k` average logs/sec and `~15.2s` body/receipt p95 versus `~204k` and `~12.1s` on the accepted run), so it was reverted and the accepted binary was redeployed.
...
- Challenge: Low peer count persisted despite the VPS gateway being healthy.
  - Resolution: Verified routing and NAT end to end, then isolated the issue to candidate-to-serving conversion and body/receipt request tails. Kept a narrower request-sizing change for unproven and slow peers after the Mac mini benchmark improved p95 latency and recent logs/sec.
...
- Inspected the peer request-sizing path in `crates/logex-sync/src/p2p/peer_manager/{mod.rs,state.rs}` after the latest low-peer investigation. No rejected dial-limit or ready-peer-capacity code remains; the kept change is limited to effective request limits for unproven peers and adaptive latency watermarks.
- Reverted and redeployed the uncommitted `2s` body/receipt hedge-delay retest after it regressed logs/sec and p95 latency on the Mac mini benchmark; no code from that retest remains.
- Reverted and redeployed the uncommitted `12`-serving-peer medium lookahead threshold retest after it regressed logs/sec and body/receipt p95 latency; no code from that retest remains.
