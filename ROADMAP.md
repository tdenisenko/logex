...
IPv6 P2P validation is materially complete for socket and address-family behavior on DigitalOcean droplet `root@152.42.222.119` with LogEx bound to IPv6 address `2400:6180:0:d2:0:2:fa9c:1000`. CL discovery and libp2p work over strict IPv6: public proofs use the built-in checkpoint quorum, IPv6 DNS, `--grpc-host ::1`, and an owner firewall rejecting IPv4 egress for the LogEx runtime user; the latest bounded proof sampled 4 times, kept `p2p_listen_families`, `p2p_advertised_families`, and `p2p_dial_families` at `["ipv6"]`, reached 71 active CL sessions and 2,400 dialable CL peers, and showed zero LogEx IPv4 sockets or IPv4-mapped log entries. A prior longer proof on the same branch reached 87 active CL sessions and 3,573 dialable CL peers. EL IPv6 transport is correct in controlled conditions: the latest two-node proof established execution sessions between two LogEx instances over an explicit IPv6 `enode://` bootnode with IPv4 egress blocked; the seed reported `connected=1`/`accepted_sessions=1`, the client reported `connected=1`/`accepted_sessions=2`, and the logs contained Eth70 activity with zero IPv4-mapped addresses. Signed ENR bootnodes are parsed, converted into family-compatible direct RLPx candidates when they advertise `ip6` plus either `tcp6` or generic `tcp`, and passed into Reth discv5 only when they advertise compatible UDP fields. Configured `enode://` bootnodes may use IP literals or DNS names; DNS names are resolved at startup into concrete A/AAAA direct-dial targets and then filtered by the selected outbound dial families. DNS-discovered execution ENRs follow the same split: direct RLPx candidates use geth/Nethermind-compatible generic TCP/UDP fallback for IPv6 endpoints, while signed discv5 seeds still require same-family UDP because the discv5 library validates IPv6 sessions against signed `udp6` socket fields. LogEx now polls the Reth DNS discovery service directly, widens the DNS record cache for the current public tree size, queues initial DNS IPv6 bootnodes as direct RLPx candidates instead of only seeding discovery, continues polling DNS in a background task after startup, and disables Reth discv4 for IPv6 binds so strict IPv6 mode does not open an IPv4 discovery socket. Public mainnet EL historical sync is still not proven because public IPv6 execution peers are effectively unavailable from the test droplet: a full geth DNS-tree crawl found 3,000 execution ENRs, 142 IPv6-capable ENRs, and only 2 TCP-reachable IPv6 endpoints; seeding those two explicit signed ENRs still produced zero accepted Eth sessions. The latest public proof stayed at 45 strict-IPv6 EL DNS candidates, 45 expired submitted dials, zero accepted public execution sessions, zero historical floor, and the derived nested `execution_network.bootstrap_warning`; longer runtime, higher peer limits, the background DNS poller, and discv5-only strict IPv6 sockets did not produce usable public EL peers. A fresh bounded official geth `v1.17.4` comparison on the same droplet formed zero peers across six 30-second samples while advertising the droplet IPv6 with IPv4 egress rejected for the geth runtime user, matching LogEx's public EL result. Default `--nat any` now selects locally owned public IPv4 first, locally owned public IPv6 second, and outbound-only otherwise; the IPv6 public-address classifier now requires global-unicast `2000::/3` and rejects special-purpose ranges before advertising an address. Automatic mode separates the advertised public address from outbound dial families: dual-public hosts advertise IPv4 while accepting IPv4 and IPv6 outbound candidates, and public-IPv6/private-IPv4 hosts advertise IPv6 while still accepting IPv4 outbound candidates. The outbound dial-family policy is applied to both EL and CL peer selection, dual-family EL startup keeps IPv6 DNS direct RLPx candidates even when discovery is bound to IPv4, and strict IPv6 wildcard selection is narrowed to a concrete local public IPv6 address when that address is verified locally so Linux does not open dual-stack wildcard P2P listeners. The known-peer cache now persists proven serving peers first and also keeps connected dialable peers that completed Eth handshake and advertised a usable tip, improving restart fallback for outbound-only and sparse-family networks without retaining raw DNS candidates. `/status` reports listen, advertised, and dial families separately so this behavior is explicit instead of implying true dual inbound support. Strict IPv6-only mode now reports a startup/status warning when no explicit IPv6 execution bootnodes are configured and also reports repeated dial-expiration warnings when configured or discovered execution candidates never become accepted sessions. The default checkpoint source is now a 2-of-3 IPv6-capable mainnet quorum, and checkpoint resolution supports Beacon headers, block/root fallback for any block id, and finalized state checkpoint endpoints. The temporary droplet clients are stopped after bounded proof windows, and no full sync is left running on the droplet.

Latest bounded droplet proof on commit `1b4af274` rebuilt the Linux binary in `8m13s` after rsyncing the local Cargo source cache and reran strict IPv6 with non-loopback IPv4 egress rejected for `logexv6`. Public mode stayed externally IPv6-only (`p2p_listen_families`, `p2p_advertised_families`, and `p2p_dial_families` all `["ipv6"]`; zero IPv4 sockets after moving gRPC to `::1`; zero reject-rule packets during a clean 45 second interval), CL reached 61 active sessions, 20 connected sessions, and 2,768 dialable peers, and public EL again expired 41 IPv6 DNS dials with zero accepted public execution sessions. A controlled explicit-bootnode proof then ran two LogEx instances on separate IPv6 EL/CL/discv5 ports; the client loaded one configured IPv6 `enode://` bootnode, reported `connected_peers = 1` and `accepted_sessions = 2`, and the socket/log snapshots showed an established Eth70 session between `[2400:6180:0:d2:0:2:fa9c:1000]:30303` and `[2400:6180:0:d2:0:2:fa9c:1000]:38362` with zero non-loopback IPv4 packets during the clean interval. Cleanup restored DigitalOcean DNS, removed the owner firewall rule, and left no LogEx process or proof listener running. This proves strict IPv6 CL operation, socket cleanliness, address-family reporting, explicit IPv6 bootnode handling, and EL IPv6 transport on the latest code, but still does not prove public strict-IPv6 historical EL sync because usable public IPv6 execution peers were unavailable.
...
- Rebuilt the current branch on the IPv6 droplet, reran strict public IPv6 under an IPv4-restricted runtime user, and confirmed CL progress plus IPv6-only external socket behavior while public EL remained blocked by sparse/unreachable IPv6 execution peers.
- Reconfirmed controlled EL IPv6 transport with two LogEx nodes and an explicit IPv6 `enode://` bootnode; the client established an Eth70 execution session over IPv6 with no ongoing non-loopback IPv4 traffic.
- Cleaned up the temporary droplet runtime state after testing: stopped all LogEx sessions, removed the owner IPv4 reject rule, and restored DigitalOcean DNS resolver settings.
...
- Added geth-compatible hostname support for configured execution `enode://` bootnodes: DNS names now resolve to concrete A/AAAA direct-dial candidates at startup before the existing dial-family filter is applied.
- Validated the hostname bootnode path with focused parser/startup tests, IPv6 DNS tests, automatic P2P selection tests, `cargo check -p logex-sync -p logex-node`, `cargo clippy -p logex-sync -p logex-node -- -D warnings`, and the full `logex-sync` test suite.
- Rebuilt the latest branch on the IPv6 droplet and reran bounded strict-IPv6 proofs with IPv4 egress rejected for `logexv6`: public mode stayed socket-clean and CL reached 81 active sessions, while public EL still expired 45 IPv6 DNS dials; the controlled hostname-enode proof established active IPv6 execution sessions on both nodes and left no running proof process after cleanup.
...
- Earlier tested Reth discv4 with family-aware IPv6 DNS pre-seeding, then superseded that path after socket proofs showed strict IPv6 execution binds must disable discv4 to avoid IPv4-capable discovery sockets.
...
- Tightened automatic public IPv6 detection so LogEx only advertises global-unicast IPv6 addresses and rejects Teredo, benchmarking, ORCHID, documentation, 6to4, translation, ULA, link-local, multicast, and non-global ranges.
- Updated `README.md` with the IPv6-relevant P2P flags: `--p2p-bind-ip`, `--execution-bootnode`, `--execution-discv5-port`, and the safer `--nat any` behavior.
- Rebuilt the current branch on the temporary IPv6 droplet and reran a bounded strict-IPv6 proof with IPv4 egress rejected for the runtime user: CL reached 75 active sessions and 3,164 dialable peers, no IPv4 or IPv4-mapped dials appeared, 46 EL IPv6 DNS candidates were submitted and expired, and no public EL session was accepted.
- Stopped and removed the temporary droplet runtime data, copied binary, source/build tree, proof logs, and IPv4 reject rule after the proof.
...
- Added the same explicit outbound dial-family policy to CL peer selection so dual-route modes retain both IPv4 and IPv6 consensus dial addresses while strict modes still filter to one family.
...
- Split initial execution DNS candidates into direct RLPx bootnodes and discovery bootnodes so dual-family IPv4-advertised startup can still queue IPv6 DNS direct dials, while discovery seeds remain compatible with the bound UDP family.
- Narrowed IPv6 wildcard P2P binds to the concrete local public IPv6 address when LogEx can verify that address locally, preventing Linux from opening dual-stack wildcard execution listeners in strict IPv6 mode.
- Rebuilt and reran the strict IPv6 droplet proof after the bind fix: `/status` reported `p2p_bind_ip = 2400:6180:0:d2:0:2:fa9c:1000`, `ss` showed `SOCKET4_COUNT=0`, execution and CL P2P listeners on the concrete IPv6 address with `v6only:1`, CL reached 78 active sessions, and public EL remained blocked only by expired IPv6 execution candidates.
- Confirmed the temporary strict IPv6 proof cleaned up after itself: no LogEx process, no owner firewall rule, no temporary data dir, no P2P ports listening, and `/etc/resolv.conf` restored.
...
- Rebuilt the current branch on the IPv6 droplet and ran a fresh strict IPv6-only public proof with IPv4 egress rejected for the LogEx runtime user. The run stayed socket-clean (`SOCKET4_COUNT=0`, `::ffff:` count `0`), CL reached 9 active sessions and 893 dialable peers, EL submitted 45 IPv6 DNS candidates, all 45 expired, and no execution session was accepted.
- Removed the latest temporary strict IPv6 proof data, script, copied runtime binary, resolver override, and owner firewall rule from the droplet after validation.
- Updated known-peer persistence so the restart cache keeps productive serving peers first, then connected dialable peers that completed Eth handshake and advertised a non-zero tip.
- Rebuilt the current branch on the IPv6 droplet and reran a bounded strict IPv6 public proof: LogEx bound EL and CL listeners to the concrete droplet IPv6 address, `SOCKET4_COUNT=0`, CL reached 60 active sessions, EL submitted 45 IPv6 DNS candidates and all expired with zero accepted sessions, and `/status` reported the execution bootstrap warning.
- Stopped the temporary strict IPv6 proof process, removed the copied binary and proof data, restored the resolver symlink, and confirmed no owner firewall rule remained.
- Moved execution DNS discovery into a bounded background poller after initial bootnode collection so DNS crawling continues independently of sync retry/backoff timing.
- Rebuilt and reran a 5 minute strict IPv6 proof on the droplet after the poller change: CL held 55-69 active IPv6 sessions, `SOCKET4_COUNT=0`, EL remained at 45 submitted/expired IPv6 DNS candidates with zero accepted sessions, and the temporary process, binary, proof data, resolver override, and owner firewall rule were removed afterward.
- Disabled Reth discv4 discovery for IPv6 execution binds and reran a socket-focused strict IPv6 proof: `ss -4` showed no LogEx sockets, `ss -6` showed the EL/CL listeners and CL peers on IPv6, CL reached 73 active sessions, EL submitted 45 IPv6 DNS candidates that all expired, and all temporary droplet artifacts were removed.
- Ran a 20 minute bounded strict IPv6-only proof with `--max-peers 160`: CL rose from 71 to 82 active IPv6 sessions and 2,333 to 3,708 dialable peers, `ss -4` showed no LogEx sockets, EL stayed pinned at 45 DNS candidates and 45 expired dials with zero accepted sessions, and all temporary droplet artifacts were removed.
- Ran a fresh official geth `v1.17.4` strict-IPv6 comparison on the same droplet with IPv4 egress rejected for the runtime user; geth advertised the droplet IPv6 but stayed at zero peers for all six 30-second samples, and the temporary tooling/data were removed afterward.
- Cleaned up stale strict-IPv6 discv4 wording in CLI help, README option docs, test text, and the roadmap so operator-facing docs now match the current discv5/direct-RLPx strict IPv6 implementation.
- Fixed initial execution DNS direct-candidate selection for dual-family startup: IPv4-bound auto mode now supplements with IPv6 direct candidates, and IPv6-bound auto mode now supplements with IPv4 direct candidates when outbound IPv4 routing exists.
- Rebuilt the current branch on the temporary IPv6 droplet using the preserved remote target cache; the release build completed incrementally in 41 seconds.
- Ran an 8 minute strict public IPv6 proof on the droplet with IPv4 egress rejected for `logexv6`: 16 samples showed zero IPv4 LogEx sockets, strict IPv6 listen/advertise/dial status, CL peaked at 94 active sessions and 3,375 dialable peers, and public EL remained blocked by 45 expired IPv6 DNS dials.
- Ran a controlled two-node strict IPv6 EL proof on the same binary using an explicit IPv6 `enode://` bootnode: seed and client both used IPv6 bind/dial families, reported execution sessions, logged Eth70 activity, and showed zero IPv4-mapped addresses.
- Restored the droplet resolver to the systemd-resolved stub, removed temporary proof data dirs and copied binaries, and verified no LogEx process or owner IPv4 reject rule remained.
...
1. Decide the production policy for strict IPv6-only EL sync.
   ...
   - Completion criteria: demonstrate EL historical progress with only IPv6 sockets against reliable public IPv6 execution peers, or make a deliberate product decision to scope strict IPv6-only EL as an advanced/explicit-bootnode mode while default startup uses proven IPv4/dual-family/outbound-known-peer paths when available.
...
- Configured execution enode bootnodes may use DNS names.
  - Why: geth resolves static enode hostnames before dialing, and operators commonly publish stable DNS names instead of raw IP literals. Supporting hostnames makes explicit IPv6 bootstrapping practical without requiring operators to update config every time a peer address changes.
  - Alternatives considered: require signed ENRs or IP-literal enodes only; rejected because it is stricter than geth and makes controlled IPv6 bootstrapping harder to operate.
  - Tradeoff: hostname bootnodes are resolved at startup into concrete A/AAAA direct-dial targets, so long-running DNS changes require a restart or future dynamic resolver support.
...
- Strict IPv6 wildcard selection narrows to a concrete local public IPv6 address when possible.
  ...
  - Tradeoff: hosts with unusual IPv6 NAT/proxying that advertise a non-local IPv6 address keep the wildcard bind and receive a warning instead of failing startup.
...
- DNS discovery is polled directly and initial DNS bootnodes are queued as direct candidates.
  - Why: Reth's bounded `node_record_stream()` listener can drop DNS updates under a large public tree, and the initial IPv6 DNS records were otherwise only used as discovery seeds instead of being sent to the direct RLPx scheduler. Dual-family startup also needs direct DNS candidates selected by outbound dial families, while discovery seeds must stay compatible with the bound UDP family.
  ...
  - Tradeoff: LogEx now owns more DNS-service plumbing, but the behavior is explicit, testable, and keeps the direct candidate path consistent with the later DNS event path without passing incompatible ENRs into Reth discovery.

- DNS discovery continues in a bounded background poller after startup.
  - Why: strict IPv6 public EL candidate discovery must not depend on when the sync loop happens to call peer refill during no-peer backoff.
  - Alternatives considered: increase refill wait budgets; rejected because it couples discovery progress to sync retry timing and can slow startup on sparse networks.
  - Tradeoff: one extra managed task and bounded channel are kept while the peer manager is alive; shutdown aborts the task with the other network tasks.

- Initial DNS direct candidates supplement the bound family in dual-family auto mode.
  - Why: a public-IPv6/private-IPv4 host advertises IPv6 and disables discv4 for socket cleanliness, but it may still have working IPv4 outbound routing. In that case, initial EL DNS direct dials must include IPv4 candidates instead of depending only on sparse public IPv6 execution peers.
  - Alternatives considered: always prefer IPv6 direct candidates when IPv6 is allowed; rejected because it strands public-IPv6/private-IPv4 hosts on the sparse public IPv6 EL peer set even though outbound IPv4 could make sync viable.
  - Tradeoff: LogEx still advertises one public/listener family, but uses the other routed family for direct outbound candidates when it improves reachability.

- Strict IPv6 execution binds use discv5 plus direct dials, not discv4.
  - Why: Reth discv4 can open an IPv4 UDP socket even when LogEx's P2P policy is strict IPv6; direct RLPx candidates from DNS/enodes and Reth discv5 cover the IPv6 paths without that socket.
  - Alternatives considered: keep discv4 and rely on firewall rejection; rejected because strict IPv6 mode should be clean at the OS socket layer.
  - Tradeoff: public IPv6 peer discovery depends on discv5 and direct DNS/enode candidates instead of discv4, which is acceptable because discv4 is not a true IPv6 discovery path.
...
- Automatic IPv6 advertising requires global-unicast addresses only.
  - Why: special-purpose IPv6 ranges can look syntactically routable but are not valid public P2P identities.
  - Alternatives considered: continue maintaining an exception list without a global-unicast gate; rejected because it can miss future or uncommon non-public prefixes.
  - Tradeoff: uncommon experimental IPv6 deployments must use explicit configuration, while normal public IPv6 hosts are selected safely.
...
- Challenge: Reth still attempted IPv4 EL dials during earlier IPv6-only discovery experiments.
...
- Challenge: execution DNS discovery may have been coupled to sync no-peer backoff timing.
  - Resolution: moved post-startup DNS discovery into a bounded background poller and abort it during peer-manager shutdown. The follow-up 5 minute strict IPv6 proof stayed socket-clean and kept CL healthy, but EL remained fixed at the same 45 submitted/expired public IPv6 candidates.
  - Remaining: public EL IPv6 peer reachability is still the blocker; DNS under-polling is not supported by the latest proof.

- Challenge: strict IPv6 still showed one wildcard UDP socket in an unfiltered `ss` sample.
  - Resolution: identified Reth discv4 as the remaining IPv4-capable discovery path and disabled discv4 for IPv6 execution binds. The follow-up proof used `ss -4` directly and showed no LogEx IPv4 sockets.
  - Remaining: resolved for socket cleanliness; public EL sync still lacks reachable IPv6 execution peers.

- Challenge: a longer strict IPv6-only run might have been needed for execution discv5 to find usable peers.
  - Resolution: ran a 20 minute bounded proof with higher EL/CL peer limits after discv4 was disabled. CL continued discovering thousands of IPv6 peers, while EL never moved beyond the same 45 DNS candidates and zero accepted sessions.
  - Remaining: public strict IPv6 EL sync remains blocked by peer-source reachability, not by short proof duration or local socket configuration.

- Challenge: generic `udp` fields in IPv6-capable execution ENRs looked like a possible missed discv5 source.
  - Resolution: inspected the local discv5 dependency and confirmed IPv6 session verification uses signed `udp6_socket()` for IPv6 node addresses. Direct RLPx dials can safely use generic TCP/UDP fallback, but signed discv5 seeds without `udp6` cannot be treated as IPv6 discovery seeds without invalidating the signed ENR.
  - Remaining: no safe code change was made; public strict IPv6 EL still requires real IPv6 execution peers or explicit reachable bootnodes.
...
- Challenge: the latest current-branch public strict IPv6 proof again produced no accepted EL session.
  - Resolution: rebuilt the branch on the IPv6 droplet, ran LogEx as a restricted runtime user with IPv4 egress rejected, temporary IPv6 DNS, `--p2p-bind-ip ::`, and `--nat extip:2400:6180:0:d2:0:2:fa9c:1000`; the run stayed socket-clean, CL reached 75 active sessions and 3,164 dialable peers, 46 EL IPv6 DNS candidates expired without an accepted session, and `/status` surfaced the nested repeated-expiration bootstrap warning.
  - Remaining: unresolved for public historical sync; the implementation behaves correctly at the socket/filtering layer, but there is still no reliable public IPv6 execution peer source from this droplet.
...
- Challenge: automatic IPv6 fallback could treat special-purpose IPv6 ranges as valid public P2P identities.
  - Resolution: tightened the IPv6 classifier to require global-unicast `2000::/3` and reject known special-purpose prefixes before advertising an IPv6 address.
  - Remaining: resolved for automatic selection; public strict IPv6 EL historical sync still depends on reliable IPv6 execution peers.
...
- Challenge: the previous single-family filter dropped IPv6 outbound candidates on hosts whose primary advertised address was IPv4.
  - Resolution: added an explicit outbound dial-family policy so EL known peers, DNS candidates, configured execution bootnodes, and CL dial addresses can be filtered by allowed outbound families rather than only the listener bind family.
...
- Challenge: initial EL DNS collection was still partly bind-centric after adding explicit outbound dial families.
  - Resolution: split DNS startup candidates into outbound-family direct RLPx records and bind-family discovery records, so dual-family IPv4-advertised mode can immediately dial IPv6 DNS candidates without feeding IPv6-only records to an IPv4 discovery socket.
  - Remaining: public strict IPv6 EL historical sync still depends on reliable IPv6 execution peers.

- Challenge: strict IPv6 tests with `--p2p-bind-ip ::` still showed Linux execution sockets with `v6only:0`.
  - Resolution: narrowed verified local public IPv6 selections to the concrete IPv6 address before starting EL/CL P2P. The follow-up droplet proof showed concrete IPv6 P2P listeners with `v6only:1`, `SOCKET4_COUNT=0`, and no IPv4-mapped log entries.
  - Remaining: public strict IPv6 EL historical sync still depends on reliable IPv6 execution peers.
...
- Challenge: public strict IPv6 EL sync still does not advance despite clean sockets and healthy CL.
  - Resolution: reran the current branch with IPv4 egress rejected, confirmed strict IPv6 status/socket behavior, and separately proved EL transport with a controlled explicit IPv6 bootnode. The remaining public-run failure is peer availability, not local IPv6 transport.
  - Remaining: production policy for strict IPv6-only EL needs either reliable public IPv6 execution peers or explicit documentation that strict IPv6 EL requires configured IPv6 execution bootnodes.
...
- Rechecked the latest strict IPv6 proof cleanup; no LogEx process, copied runtime binary, proof data dir, proof script, owner firewall rule, resolver override, or synced build tree remains on the droplet after the bounded test.
- Rechecked automatic P2P address selection and CLI documentation; no dead runtime paths were introduced, and the only code change is the stricter IPv6 public-address classifier plus focused test coverage.
- Rechecked CL dial filtering after adding explicit outbound families; the obsolete bind-family-only retention path was replaced, and focused tests cover IPv4-only, IPv6-only, and dual-family filtering.
- Rechecked EL initial DNS candidate handling after splitting direct and discovery records; the obsolete bind-family-only initial direct path was replaced, and focused tests cover dual-family IPv6 direct selection plus UDP-gated discovery seeding.
- Rechecked IPv6 address selection after replacing wildcard strict binds with verified concrete IPv6 binds; no dead code was introduced, and tests cover automatic/public IPv6 narrowing plus the warning path for non-local external IPv6 addresses.
- Rechecked the background DNS poller change; it replaces no production API, adds a bounded task that is aborted during peer-manager shutdown, and the temporary local proof runner plus remote proof data were removed after validation.
- Rechecked the strict IPv6 discv4 disablement; it removes the remaining IPv4-capable discovery socket path for IPv6 execution binds without changing direct RLPx or discv5 handling, and the temporary socket-proof runner plus remote proof artifacts were removed after validation.
- Rechecked the 20 minute strict IPv6 proof cleanup; no LogEx process, copied runtime binary, proof data dir, proof script, owner firewall rule, resolver override, or local temporary proof runner remains after validation.
- Rechecked the geth comparison cleanup; no geth process, temporary datadir, IPC file, owner firewall rule, or downloaded alltools directory remains on the droplet.
- Rechecked docs and tests for obsolete strict-IPv6 discv4 references; removed the superseded roadmap decision and updated help/readme/test wording where it implied current strict IPv6 mode still uses discv4.
- Rechecked initial DNS direct-candidate selection for dual-family startup; replaced the IPv6-only helper with a bound-family-aware helper and added focused tests for IPv4-bound plus IPv6-direct and IPv6-bound plus IPv4-direct behavior.
...
- Rechecked the latest droplet proof artifacts; they are operational test data only, not project code. The temporary LogEx processes were stopped, the owner IPv4 reject rule was removed, and resolver settings were restored. The droplet still contains the rsynced source/build cache only because it is temporary and scheduled for removal by the user.
...
- Commits made this run: `docs: record final ipv6 proof status`; `fix: resolve hostname execution bootnodes`; `docs: record latest ipv6 proof`; `docs: record ipv6 discovery audit`; `fix: warn on nonserving execution sessions`; `docs: record droplet ipv6 proof`.
...
- Automatic single-advertised-family selection is implemented for public IPv4, public IPv6 fallback, and outbound-only fallback, and outbound dials can use both families when both routes exist. True simultaneous IPv4+IPv6 inbound/listener operation is not implemented yet.
