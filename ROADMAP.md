...
- Challenge: the latest current-branch public strict IPv6 proof again produced no accepted EL session.
  - Resolution: rebuilt the branch on the IPv6 droplet, ran LogEx as a restricted runtime user with IPv4 egress rejected, temporary IPv6 DNS, `--p2p-bind-ip ::`, and `--nat extip:2400:6180:0:d2:0:2:fa9c:1000`; the run stayed socket-clean and surfaced the repeated-expiration bootstrap warning.
...
- Rechecked the latest strict IPv6 proof cleanup; no LogEx process, copied runtime binary, proof data dir, proof script, owner firewall rule, or resolver override remains on the droplet after the bounded test.
...
- Commits made this run: pending `fix: tighten public ipv6 address detection`.
