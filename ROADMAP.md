...
IPv6 P2P validation is materially complete for socket and address-family behavior on DigitalOcean droplet `root@152.42.222.119` with LogEx bound to IPv6 address `2400:6180:0:d2:0:2:fa9c:1000`. CL discovery and libp2p work over strict IPv6: the latest public proof used the built-in checkpoint quorum, IPv6 DNS, `--grpc-host ::1`, and an owner firewall rejecting IPv4 egress for the LogEx runtime user; across 18 status samples it reached 84 active CL sessions, 3,575 dialable CL peers, zero IPv4 LogEx sockets, and zero `::ffff:` IPv4-mapped dial log entries. EL IPv6 transport is correct in controlled conditions: the current branch established Eth70 execution sessions between two LogEx instances over IPv6 only using both `enode://` and signed `enr:` execution bootnodes, with IPv4 egress blocked for the runtime user and zero matching IPv4 sockets. Signed ENR bootnodes are parsed, converted into strict-family direct RLPx candidates when they advertise compatible TCP fields, and passed into Reth discv5 as signed discovery seeds when they advertise compatible UDP fields. DNS-discovered execution ENRs follow the same split: direct RLPx candidates still require family-compatible TCP fields, while IPv6 UDP-only ENRs can seed discv5 without being misclassified as direct dial targets. Public mainnet EL historical sync is still not proven because public IPv6 execution candidates are not usable enough: the latest strict IPv6-only public run rejected 380 DNS execution records for the selected IPv6-only dial family, rejected 4 records by fork id, and established zero execution sessions. Earlier fallback probes that dialed `ip6` plus shared-`tcp` records timed out or hit closed ports, so LogEx avoids those wasted dials. A bounded official geth `v1.17.4` comparison on the same droplet also formed zero peers with IPv4 egress blocked; with `--netrestrict 2000::/3`, geth filtered every default execution bootstrap node as IPv4-only, and without netrestrict it kept trying IPv4 candidates but still reached zero peers. Default `--nat any` now selects locally owned public IPv4 first, locally owned public IPv6 second, and outbound-only otherwise. Automatic mode separates the advertised public address from outbound dial families: dual-public hosts advertise IPv4 while accepting IPv4 and IPv6 outbound candidates, and public-IPv6/private-IPv4 hosts advertise IPv6 while still accepting IPv4 outbound candidates. `/status` reports listen, advertised, and dial families separately so this behavior is explicit instead of implying true dual inbound support. Strict IPv6-only mode reports a startup/status warning when no explicit IPv6 execution bootnodes are configured. The default checkpoint source is now a 2-of-3 IPv6-capable mainnet quorum, and checkpoint resolution supports Beacon headers, block/root fallback for any block id, and finalized state checkpoint endpoints. The temporary droplet clients are stopped after bounded proof windows, and the latest proof artifacts, source tree, copied binary, test user, and firewall rule were removed after validation.
...
- Tightened family-aware EL DNS conversion so strict IPv6 direct dials require `ip6 + tcp6`, and IPv6 discovery seeds require `udp6`, matching Reth's IPv6 discovery behavior instead of treating IPv4 ENR ports as IPv6 RLPx endpoints.
- Rebuilt the current branch on the IPv6 droplet and reran a bounded strict IPv6 proof with IPv4 egress rejected for the LogEx runtime user; CL reached live head materialization over IPv6, while EL rejected `379` DNS records for lacking a usable IPv6 RLPx endpoint and accepted no execution sessions.
...
- Added signed `enr:` support to `--execution-bootnode`/`execution_bootnodes`; signed ENRs now seed discv5 directly and also produce strict-family direct RLPx candidates when their TCP fields match the selected outbound family.
...
- Split DNS-discovered execution ENRs into strict-family direct RLPx candidates and signed discv5 bootstrap seeds, so IPv6 UDP-only ENRs are no longer discarded just because they cannot be direct TCP dials.
...
- DNS-discovered signed ENRs can seed discv5 without becoming direct RLPx candidates.
  - Why: discv5 needs family-compatible UDP fields, while direct RLPx dials require family-compatible TCP fields. Treating both as the same candidate type either drops useful discovery seeds or wastes direct dial slots.
  - Alternatives considered: keep rejecting ENRs without IPv6 TCP; rejected because it prevents future IPv6 UDP-only discovery seeds from bootstrapping the routing table.
  - Tradeoff: current public EL IPv6 peer availability is still sparse, so this fixes the local classification bug but does not by itself prove public strict IPv6 historical sync.
...
- Family-aware DNS no longer maps IPv4 ENR TCP/UDP ports onto IPv6 execution endpoints.
  - Why: Reth's IPv6 discovery path expects `tcp6` for IPv6 RLPx, and the public records that only had `ip6` plus shared/IPv4 TCP fields were closed or timed out in direct probes.
  - Alternatives considered: keep the fallback and continue submitting speculative dials; rejected because it wastes dial budget and produces misleading "IPv6 candidate accepted" counters.
  - Tradeoff: strict IPv6-only mode now rejects more public DNS records upfront; users need explicit IPv6 execution bootnodes until the public EL peer source contains usable `tcp6` endpoints.
...
- Challenge: the DNS path previously counted `ip6` plus shared/IPv4 TCP records as IPv6 EL candidates.
  - Resolution: aligned LogEx's family-aware DNS conversion with Reth's IPv6 discovery behavior and require `tcp6` for strict IPv6 RLPx direct dials; the follow-up droplet proof rejected all public DNS records before wasting dial slots and kept CL healthy over IPv6.
  - Remaining: public mainnet EL strict IPv6 still needs real IPv6 execution bootnodes or another candidate source.

- Challenge: DNS ENRs with compatible IPv6 UDP but no compatible IPv6 TCP would have been unusable.
  - Resolution: split DNS ENR handling so `ip6 + udp6` records seed Reth discv5 as signed ENRs, while only `ip6 + tcp6` records become direct RLPx candidates.
...
- Rechecked the IPv4-mapped CL dial-filter change; it is limited to address-family classification and focused tests. The latest droplet proof left no LogEx/geth/reth process, owner firewall rule, source tree, copied binary, proof data, proof log, or test user behind.
