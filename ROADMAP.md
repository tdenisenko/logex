...
IPv6-only mode is functionally validated on temporary droplet `root@152.42.222.119` with public IPv6 `2400:6180:0:d2:0:2:fa9c:1000`. Strict IPv6 CL networking works with IPv6 DNS, `--grpc-host ::1`, IPv6-only P2P bind/dial settings, and an owner firewall rejecting IPv4 egress for the LogEx runtime user. Deterministic two-node EL proofs on the latest branch establish execution sessions over IPv6 with zero IPv4 sockets, and a prior current-HEAD public proof accepted a serving public EL peer, tracked live head, and moved historical sync backward. Fresh public strict-IPv6 windows still vary because public EL IPv6 peers are sparse, so the README recommends reliable IPv6 EL bootnodes or a warmed known-peer cache for production strict IPv6 runs.
...
- Challenge: the current-HEAD strict IPv6 proof did not quickly find an EL session from DNS-only discovery or a stale known IPv6 enode.
  - Resolution: refreshed the TCP-open IPv6 enode set from the droplet, supplied the reachable enodes explicitly, and confirmed one bounded run accepted a serving public EL peer, reached `Syncing`, and moved historical sync with only IPv6 sockets. Later fresh bounded windows accepted strict-IPv6 candidates and stayed socket-clean but did not reproduce a serving EL session.
