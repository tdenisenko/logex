...
- Re-ran strict IPv6 droplet smoke after the CL bootnode filter: CL peaked at 71 active sessions and 2,217 dialable peers with zero LogEx IPv4 sockets, while public EL submitted 41 IPv6 dials and all expired.
- Fixed sparse EL candidate handling so submitted direct dials retain their `NodeRecord` and are requeued after the existing submitted-dial suppression window if Reth produces no session event.
- Rebuilt and reran strict IPv6 smoke after the retry fix: CL peaked at 86 active sessions and 2,639 dialable peers, `socket4_count` stayed `0`, and EL submitted dials rose from 41 to 248 over five samples instead of going idle after the first expiration wave.
- Verified the retry change with `cargo test -p logex-sync expired_submitted_dials_return_nodes_for_retry -- --nocapture`, `cargo test -p logex-sync p2p::peer_manager -- --nocapture`, `cargo check -p logex-sync`, `cargo clippy -p logex-sync -- -D warnings`, and remote bounded IPv6 smoke.
