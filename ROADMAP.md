...
- Challenge: the paired chunk path waited for the first body and receipt futures to both finish before retrying the slow side.
  ...
  - Remaining: extend live ownership across chunk attempts and plan windows if further benchmarking shows prefix stalls remain.
...
- Validation run this pass: `cargo fmt --check`; `cargo check -p logex-sync`; `cargo test -p logex-sync`; `cargo clippy -p logex-sync -- -D warnings`; `sh -n local-ops/sample-logex-throughput.sh`; corrected sampler validation against the Mac mini. The accepted bounded per-chunk live role retry change passed `cargo fmt --check`, `cargo test -p logex-sync`, and `cargo clippy -p logex-sync -- -D warnings`, then was live-tested on the Mac mini at `/Users/gremlinmaster/logex-src/run/logex-throughput-v3-20260625-064959.log`.
