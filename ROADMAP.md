...
The latest accepted slice is a larger live-scheduler change: one validated reverse-header window can now materialize multiple deterministic, non-overlapping body/receipt fetch plans. This keeps more verified work available to the downloader without revalidating the same header suffix, and the body/receipt plan cap prevents speculative lookahead from overlapping the ordered prefix. A five-minute Mac mini sample showed about 347 actual blocks/sec, no zero-progress windows, and local physical RX as the active constraint. The feature is still not complete because active fetch slots can briefly drain to zero; the next scheduler work is explicit expected-sequence priority and refill reservation, not more timeout/chunk-size tuning.
...
- Accepted multi-plan header materialization for the live scheduler:
  - Large reverse-header batches now feed several capped body/receipt plans instead of only one plan plus discarded suffix headers.
  - Each speculative body/receipt plan is truncated to its deterministic contiguous prefix, so queued lookahead cannot overlap the next sequence.
  - Local validation passed with `cargo fmt --check`, focused body/receipt cap tests, and scheduler decision tests.
  - Remote smoke on the Mac mini showed continuous floor movement with zero zero-progress windows in the five-minute sample; active-slot drain remains the next issue.
...
- Header materialization may split one validated header window into multiple capped body/receipt fetch plans. Each plan is capped to the prefix it is allowed to return, which preserves ordered verification and avoids overlapping speculative suffixes.
