...
- Removed the temporary strict IPv6 proof data, proof binary, proof scripts, resolver override, and owner firewall rule from the droplet after validation; `/root/logex-ipv6` remains only as a temporary source/build cache for another bounded proof if needed.

## Remaining TODOs

1. Decide the production policy for strict IPv6-only EL sync.
   - Reason: CL works over IPv6 and EL transport/discovery is socket/family-clean. The public execution DNS tree does contain IPv6 records and LogEx now submits them directly, but those public endpoints expired without serving sessions in bounded LogEx tests; official geth also formed no public execution peers when IPv4 egress was blocked. LogEx now warns users when strict IPv6-only execution sync has no explicit IPv6 bootnodes.
...
- Commits made this run: `fix: queue ipv6 dns bootnodes for direct dials`.
